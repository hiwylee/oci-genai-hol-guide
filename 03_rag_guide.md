...
> 아직 환경 설정이 완료되지 않았다면 [02_step_by_step.md](./02_step_by_step.md) 를 먼저 진행하세요.
...
LLM: "01_prerequisites.md 문서에 따르면, 이 실습에서 사용하는
...
```
HOL_Guide/
├── 00_README.md          ← 실습 개요, 아키텍처
├── 01_prerequisites.md   ← 개념 설명, 사전 준비
└── 02_step_by_step.md    ← 단계별 실습 명령어
        │
        ▼ (업로드)
  ...
  "Gateway config 파일이 왜 두 개인가요?"
        │
        ▼ (RAG 검색 → 답변)
  01_prerequisites.md 내용을 근거로 답변
```
...
> `00_README.md`, `01_prerequisites.md`, `02_step_by_step.md` 파일을 드래그하여 로컬 PC로 저장

> **Mac/Linux 사용자**: `scp` 명령으로 파일 다운로드:
> ```bash
> scp -i ssh-hol.key opc@<VM_IP>:/home/opc/projects/oci_genai_hol/HOL_Guide/0*.md ~/Downloads/
> ```
...
어떤 컴포넌트들이 어떻게 연결되나요?
```
> 기대 답변: 00_README.md의 아키텍처 다이어그램 내용 — Open-WebUI, Gateway, ADW, OCI GenAI 연결 구조
...
둘의 차이점은 무엇인가요?
```
> 기대 답변: 01_prerequisites.md §4 내용 — 리전 차이(춘천 vs 시카고), 목적 차이(IAM·ADW vs GenAI API 호출)
...
```
> 기대 답변: 01_prerequisites.md §8 내용 — Podman 브릿지 네트워크, 컨테이너 내부 localhost 개념
...
```
> 기대 답변: 00_README.md §1단계 — 11개 단계 목록 및 설명
...
```
> 기대 답변: 02_step_by_step.md Step 10 및 Step 3 내용 — 인증 파일 마운트, 컨테이너 내부 경로
...
그 이유는 무엇인가요?
```
> 기대 답변: 02_step_by_step.md Step 4 내용 — 바이너리 파일(cwallet.sso, ewallet.p12) vs 텍스트 파일
...
```
> 기대 답변: 02_step_by_step.md Step 8 내용 — cohere.embed-v4.0, 1024차원, Oracle ADW AI Vector Search
...
```
> 기대 답변: 02_step_by_step.md Step 8 내용 — .env 주요 항목 설명
...
무엇을 확인해야 하나요?
```
> 기대 답변: 01_prerequisites.md §4 오류 1 내용 — fingerprint, key_file 경로 불일치 확인 방법
...
어떻게 해결하나요?
```
> 기대 답변: 00_README.md 문제해결 섹션 — loginctl enable-linger 설정 확인 및 podman start 명령
...
```
@01_prerequisites  OCI API Key 인증 방식의 동작 원리를 RSA 키 교환 관점에서 설명해 주세요.
```

```
@02_step_by_step  Open-WebUI 컨테이너 실행 시 --restart=always 옵션이 없으면 어떻게 되나요?
```
...
**← 이전**: [02_step_by_step.md](./02_step_by_step.md)
**→ 다음**: [04_openwebui_tools.md](./04_openwebui_tools.md)
